...
void removeOddValues(Queue *q)
{
    ...
    //return if the q is empty
    if (q == NULL)
        return;
    
    //dequeue all the variable in queue q
    //if its even, we will queue it into temp, else we will just dequeue the next element
    while(!isEmptyQueue(q))
    {
        dequeued_var = dequeue(q);
       //printf("dequeue_var: %d",dequeued_var);

        if (dequeued_var % 2 == 0)
            enqueue(&temp, dequeued_var);
    }

    //once q is empty, we will repopulate queue q with the items in queue temp
