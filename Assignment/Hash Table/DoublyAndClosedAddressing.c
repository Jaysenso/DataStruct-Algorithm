...
{   
    ...
    //instantiate the new key and point next to NULL
    ListNode *newNode = malloc(sizeof(ListNode));
    newNode->key = 0;
    newNode->next = NULL;
    newNode->pre = NULL;

    //if the linked list is originally empty ->then we will update the head pointer to point at newNode
    ...
    //else we will update the head pointer 
    else if(Q1Ptr->Table[index].head != NULL)
    {   
        ListNode *curr = Q1Ptr->Table[index].head;
        //store the head pointer in temp
        ListNode *temp = curr;
...
{
    //search the hash to check if the key exists
    if(HashSearch(*Q1Ptr,key) == NULL)
    {
        return 0;
    }

    //locate the index where they key resides
    int index = Hash(key, Q1Ptr->hSize);
    ListNode *curr = Q1Ptr->Table[index].head;
    ListNode *prev = NULL;
    ListNode *temp;

    //assuming the head pointer aka Hashnode in the table is the key we want to delete
    if(Q1Ptr->Table[index].head->key == key)
    {   
        //we store the head pointer so that we can free it afterwards
        temp = curr;
        curr = curr->next;

        Q1Ptr->Table[index].head = curr;
        free(temp);
        return 1;
    }

    //else we are deleting element from the linked list instead
    while(curr != NULL)
    {   
        //duplicate key -> we will return -1 indicating insucessful insertion
        if (curr->key == key)
        {
            //store the key into a temp pointer so that we can free it afterwards
            temp = curr;
            ...
            //free the memory -> deleting the node
            free(temp);
            return 1;
        }
