...
void moveOddItemsToBack(LinkedList *ll)
{
    ...
    for(i = 0; i < ll->size; i++)
    {
        if(cur->item % 2 == 1) //check if current value is odd
        {   
            //store the current item and the current->next aswe are deleting the current
            item = cur->item;
            //store the current-next for traversal
            temp = cur->next;
