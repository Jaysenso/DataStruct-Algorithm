...
int removeNode2(LinkedList *ll,int index)
{
    ...
    //if removing first node, update the head pointer
    else if (index == 0)
    {
        ll->head = cur->next;
        free(cur);
    ...
    //if removing nth index node
    else
    {
        //traverse down the linked list and search for target node
        while(cur != NULL)
        {
            //once found, we connect the pre node and cur->next 
            //then we free the target node (cur)
            if(index == cur_index)
            {
                pre->next = cur->next;
                free(cur);
            ...
            //traversal double pointers where pre is always 1 index behind current
            pre = cur;
            cur = cur->next;
            cur_index++;
        }
