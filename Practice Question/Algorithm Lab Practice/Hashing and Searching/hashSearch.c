...
ListNode* HashSearch(HashTable Q3Hash, int key)
{
    ...
    if(key < 0)
        return NULL;
    //the table is empty and we can never find it in the Hashtable
    if(Q3Hash.hSize != 0){
    ...
    //then we hash the key for the pointer to the array
    ListNode *curr = Q3Hash.Table[index].head;              
    //since we cant insert duplicate key ->we just have to return the 1st appearance of the key
    while(curr != NULL)
    {
        if(curr->key == key)
        {
    ...
    //we should have found it in the while loop, else -> is not in the hashTable
    return NULL;
...
{
    //HashSearch found a matching key -> duplicate key 
