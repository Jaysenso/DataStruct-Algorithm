...
void removeDuplicatesSortedLL(LinkedList *ll)
{
    ...
    while(ptr1 != NULL && ptr1->next != NULL)
    {
        ...
        ptr2 = ptr1;
        //similar to nested for loop, ptr2 will traverse down the list looking for the duplicates
        while(ptr2->next != NULL)
        {
            //REASON WHY PTR2 = PTR1: if condition will search the next node 's value e of the current index
            if(ptr1->item == ptr2->next->item)
            {
                //store the duplicate node into temp variable "duplicate"
                duplicate = ptr2->next;
                //reconnect the node before duplicate and the one after together
