...
ALGORITHM EXPLANATION
    1. we create & initialize an empty stack (this will be used to dequeue & print the value)
    2. we will enqueue the root node (e.g : 20)
    3. enter a while condition to check that if the queue is empty
        . if its not empty, we will dequeue the first item and print it. Then we will queue its left & right childrens
        . since the 1st item (root - 20) has been dequeued, q.head will be updated to 15 (left children of 20)
        . we will update curr with 15 then the loop (step 3) continues
    4. this process will stop when we hit (root==null) our sanity check, then we know that we have reached the end
*/
void levelOrderTraversal(BSTNode* root)
{
    ...
    //create & initialize empty queue q;
    Queue q;    
    q.head = NULL;
    q.tail = NULL;

    //curr will store the current node <- this will be updated by our dequeue()
    BSTNode *curr = root;
