...
void reverseFirstKItems(Queue *q, int k)
{
    ...
    // Supposethat the q was 1,2,3,4,5,6, k = 3
    // Deque the first k elements to store into the stack
    for (i = 0; i < k; i++)
    {
        push(&s,dequeue(q));
    }

    //Queue is now 4,5,6 while stack is 3,2,1
    //Then we push all the elements in stack into the queue again
     while (!isEmptyStack(&s))
     {
        enqueue(q, pop(&s));
     }

    //The queue now is 4,5,6,3,2,1 while stack is empty
    //We will now dequeue those elements that were not touched initially and push it into the stack
    for (i = 0; i < q->ll.size - k; i++)
    {
        printf("%d %d",s.ll.size,k);
        enqueue(q, dequeue(q));
    }
