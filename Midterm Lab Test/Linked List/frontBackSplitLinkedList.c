...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
    ...
    ListNode *curr_front = resultFrontList->head;
    ListNode *curr_back = resultBackList->head;
    ...
    while(curr != NULL)
    {
        if (index <= ll->size / 2)
        {
            insertNode(&(*resultFrontList),index_front,curr->item);
            index_front++;
        }
        else if ( index > ll->size / 2)
        {
            insertNode(&(*resultBackList),index_back,curr->item);
            index_back++;
        }
        index++;
        curr = curr->next;
